...
void storeCgpDriver(vector<int> driver) {
	ofstream driverFile;
	driverFile.open("cgpDriver.txt", std::ofstream::out | std::ofstream::trunc);
...
vector<int> readCgpDriver() {
	string text;
	ifstream driverFile("cgpDriver.txt");
...
void storeNnDriver(vector<double> driver) {
	ofstream driverFile;
	driverFile.open("nnDriver.txt", std::ofstream::out | std::ofstream::trunc);
	for (int i = 0; i < driver.size(); i++) {
		driverFile << driver.at(i) << " ";
	}
	driverFile.close();
...
vector<double> readNnDriver() {
	string text;
	ifstream driverFile("nnDriver.txt");
	while (getline(driverFile, text)) {}
	driverFile.close();
